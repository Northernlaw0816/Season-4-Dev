		...
		<>
			...
			<Layout skipTo="#tagline" overrideClasses={`${styles.main} ${styles.main_ps}`}>
				...
				<div className={styles.content}>
					...
					<ul>
						{PitstopData.details.dateAndTime.map((date, index) => {
							return (
								<li key={index}>
									<p>{date}</p>
								</li>
					...
					<p className={styles.h2_p}>
						<strong>Venue:</strong> {PitstopData.details.venue}
					</p>

					<h2 id="guidelines">Guidelines</h2>
					<ul>
						{PitstopData.headings.rules.map((rule, index) => {
					...
					<ul>
						{PitstopData.headings.registration.map((rule, index) => {
							return (
								<li key={index}>
									<p>{rule}</p>
								</li>
					...
					<EventsRegisterButton pathname="https://docs.google.com/forms/d/e/1FAIpQLScIKchFZbb4zT0GkddX8nIZYiM0tugj3dtZBjdRRlbFPBhiiA/viewform" text={`Register for ${PitstopData.title}`} />
					</div>
				</div>
			</Layout>
		</>
